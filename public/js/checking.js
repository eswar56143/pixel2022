...
function resetCodeCombatCombo() {

    document.getElementById('submit_c').disabled = false;
    $('#body').css({ overflow: 'scroll' });
    document.getElementById("register-code-combat-1").reset();
    document.getElementById("register-code-combat-1").style.display = "block";
    document.getElementById("form-done_c_1").style.display = "none";
    document.getElementById("form-fail_c_1").style.display = "none";
    $("#user-availability-status").html("")
    $("#user-availability-status-event").html("");
    $("#team-size-validater").html("");
    $("#team-name-validater").html("");
...
function checkoutCD(event_name) {
    ...
    //email validation
    if (email.length == 0) {
        $("#user-availability-status").html("This field is Required");
        return;
...
function checkOut(event_name) {
    ...
    if (registered_id.length == 0) {
        $("#user-availability-status-event").html("This field is Required");
        return;
    ...
    if (item.length == 0) {
        $("#team-size-validater").html("This field is Required");
        return;
    ...
    if (teamSize == 0) {
        ...
        if (teamSize > 1 && teamName.length == 0) {
            $("#team-name-validater").html("This field is Required");
            return;
        ...
        for (let mem = 1; mem <= teamSize; mem++) {
            ...
            if (teamSize.length > 1 && (memName.length == 0 || memClglength == 0 || membranch.length == 0)) {
                $("#team-member-validater").html("This fields is Required");
                return;
            }
...
function checkOutCombo(event_name) {
    $("#user-availability-status-event").html("");
    ...
    let registered_id = document.getElementById('registered_id_t').value;
    //email validation
    if (registered_id.length == 0) {
        $("#user-availability-status-event").html("This field is Required");
        return;
                    ...
                    "handler": function(response) {
                        ...
                        $.ajax({
                            ...
                            success: function(result) {
                                if (result == 'true') {
                                    document.getElementById("register-code-combat-1").reset();
                                    document.getElementById("register-code-combat-1").style.display = "none";
                                    document.getElementById("form-done_c_1").style.display = "block";
                                } else {
                                    document.getElementById("register-code-combat-1").reset();
                                    document.getElementById("register-code-combat-1").style.display = "none";
                                    document.getElementById("form-fail_c_1").style.display = "block";
                                }
                                console.log(event_name)
